(function () {
    ...
    angular.module("latinQuiz", ['ngRoute','angularjs-datetime-picker']);
    var statuses={
    ...
    var resultsParse=function(hw){
	hw.questions.forEach(function(quest){
	    console.log(quest.qType);
	    if((quest.userAnswer!=undefined) || (quest.userAnswers!=undefined)){
		if(quest.qType.toString()=="ma"){
		    var acc=0;
		    quest.response="";
		    quest.userAnswers.forEach(function(ans){
			if(acc!=quest.userAnswers.length){
			    quest.response=quest.response+ans+",";
			}
			else{
			    quest.response=quest.response+ans;
			}
		    ...
		    quest.response=quest.userAnswer;
		}
	    }
	})
    }
	...
	//no return
        ...
        var dateToString = function(aDate){
     
   
           var month = aDate.getMonth() +1;
            var day =aDate.getDate();

            var year =aDate.getFullYear() -2000;//work for format
            var hour = aDate.getHours();//might need to add one]
            ...
            var returnString = month + '/' + day + '/' + year +' ' + hour + ':' + minutes + ' ' + hourType;
            return returnString;
       //     $scope.dateStre=$scope.dateStre.replace(/EDT|EST|CDT|CST|MDT|PDT|PST|UT|GMT/g,'');
   // $scope.dateStre=$scope.dateStre.replace(/\s*\(\)\s*/,'');
    //$scope.dateStre=$scope.dateStre.replace(/[\-\+][0-9]{2}:?[0-9]{2}$/,'');
        
        }
        var stringToDate =function(aString){
             var sample = aString.split(" ");
        var date = sample[0];
        var timeArray=sample[1].split(":");
        var hourType=sample[2];
        var dateArray = date.split("/");
        var month=parseInt(dateArray[0])-1;
        var day = parseInt(dateArray[1]);
        var year= 2000+ parseInt(dateArray[2]);
        ...
        var returnDate = new Date(year, month, day, hour, min, 0, 0);        
        return returnDate;
        }
	...
	//submits student answers
         var postStudentHomework= function(newHw){
            return $http.post("/newStudentHomework/",{HW:newHw});
        }
	
        var getNewCreatedHW = function(classId,hwId){
            return $http.get("/createHW/"+classId+"/"+hwId);
        }
	/*
	...
	var deleteHomework=function(classId,chapterId,homeworkId){
	    return $http.post("/deleteHomework/"+classId+"/"+chapterId+"/"+homeworkId);
	}
	...
	//see hwData in test.js
        var getHw = function (classId,hwId) {
            return $http.get("/hwResults/" + classId + "/"  + hwId);
        }
	...
	var setDateOfHw=function(classId,hwDataToChange){
	    ...
	    //homework id
	    //and new due date
	    return $http.post("/setDate/"+classId,{hwDataToChange:hwDataToChange});
	}
	...
	//get the data for each class
        latinQuizModel.getData().then(function (message){
	...
	*/
	...
	//http request for getting the data
        latinQuizModel.getHw($routeParams.classId,$routeParams.hwId).then(function (message){
            $scope.hw = message.data.homework;
            $scope.changeIndex(parseInt($routeParams.questionId));
        });
	...
	//http request to get info for student page
        latinQuizModel.getStudentInfo().then(function(message){
            ...
            if(response.nextHomework!=undefined){
                $scope.nextHW=response.nextHomework;
                $scope.nextHW.dateString=latinQuizModel.dateToString(new Date($scope.nextHW.dueDate));
            }
	...
	//set which pictures to display with each chapter
        $scope.adjustPic=function(chapter){
            ...
            chapter.homework.forEach(function(hw){
                hw.dateString=latinQuizModel.dateToString(new Date(hw.dueDate));
	...
	if($routeParams.hwId==-1){
	    ...
	    latinQuizModel.getNewCreatedHW($routeParams.classId,$routeParams.hwId).then(function(message){
		console.log(message.data);
		var k=message.data.generatedHomework;
		homeworkSanatize(k);
		var data=k;
		$scope.hw=data;
		$scope.t = $scope.hw.questions;
		console.log("scope.hw");
		console.log($scope.hw);
		console.log("scope.hw");
		$scope.hw.dueDate=new Date($scope.hw.dueDate);
		if(angular.isDefined($scope.hw.dueDate)){
                    $scope.convertDate();
		} 
		//this ensures that the homework is organized by the index in which they appear, so the index goes
        ...
        $scope.convertDate = function (){//turns a date into string, used by the date time picker
            //split up dates by space and : see what you can do with that
            $scope.dateStr=$scope.hw.dueDate.toDateString();
            var month = $scope.hw.dueDate.getMonth() +1;
            var day =$scope.hw.dueDate.getDate();

            var year =$scope.hw.dueDate.getFullYear() -2000;//work for format
            var hour = $scope.hw.dueDate.getHours();//might need to add one]
            var minutes = $scope.hw.dueDate.getMinutes();
            $scope.dateStre=$scope.hw.dueDate.toTimeString();
            var hourType = "AM";
            if(hour >=12){
                hourType="PM";
                if(hour>12){
            ...
            $scope.hw.dueDate= month + '/' + day + '/' + year +' ' + hour + ':' + minutes + ' ' + hourType;
       //     $scope.dateStre=$scope.dateStre.replace(/EDT|EST|CDT|CST|MDT|PDT|PST|UT|GMT/g,'');
   // $scope.dateStre=$scope.dateStre.replace(/\s*\(\)\s*/,'');
    //$scope.dateStre=$scope.dateStre.replace(/[\-\+][0-9]{2}:?[0-9]{2}$/,'');
        }
        ...
        $scope.dateStre="";
        $scope.myRealDate="";
        $scope.$watch("hw.dueDate",function(newVal,oldVal){
            if(angular.isDefined($scope.hw)){
                if($scope.hw.dueDate != ""){
                
                    if(typeof $scope.hw.dueDate == String){
                        $scope.logdate();
                    }
                //$scope.hw.dueDate=$scope.myRealDate;
                }
            }
      });
        $scope.logdate =function(){ 
            var sample = $scope.hw.dueDate.split(" ");
            var date = sample[0];
            var timeArray=sample[1].split(":");
            var hourType=sample[2];
            var dateArray = date.split("/");
            ...
            var jk = new Date();
            $scope.myRealDate= new Date(year, month-1, day, hour, min, 0, 0);        
        // $scope.dateStr=$scope.myDate.toDateString();
     //$scope.dateStre=$scope.myDate.toTimeString();
...
$scope.indexChanged = function(newVal,oldVal){
    ...
    var difference;
    var bigToSmall;
    if(newVal.index > oldVal){//small to big
    ...
    if(bigToSmall){
        ...
        var transition = $scope.hw.questions[smallerIndex];
        for(var i = smallerIndex;i<largerIndex;i++){
            $scope.hw.questions[i]=$scope.hw.questions[i+1];
            $scope.hw.questions[i].index-=1;
        }
        $scope.hw.questions[largerIndex]=transition;
        $scope.hw.questions[largerIndex].index=largerIndex+1;
    }
...
//these buttons are fairly straight forward, they just help adjust the location path
	...
	$scope.save =function(){
	    var save=true;
	    var messageToShow="";
	    $scope.hw.questions.forEach(function(quest){
		if(quest.prompt=='' || quest.prompt==undefined){
		    messageToShow="There must be a prompt in all questions";
		    save=false;
		    return;
		}
	    })
	    if($scope.hw.name=='' || $scope.hw.name==undefined){
		messageToShow="There must be a name";
		save=false;
	    }

	    if($scope.hw.dueDate=='' || $scope.hw.dueDate==undefined){
		messageToShow="There must be a duedate";
		save=false;
	    }
	    if($scope.hw.endingMessage=='' || $scope.hw.endingMessage==undefined){
		messageToShow="There must be an ending message";
		save=false;
	    }

	    if(save){
		console.log(typeof $scope.hw.dueDate);
		if((typeof $scope.hw.dueDate)=="string"){
		    $scope.hw.dueDate=latinQuizModel.stringToDate($scope.hw.dueDate);
		}
		...
		$scope.messageError=false;		
		$scope.errorMessage=messageToShow;
	    }
  ...
  return a.index - b.index;
    ...
    */
    ...
    var blankCopyController = function($scope,$location,latinQuizModel,$routeParams){
	    ...
	    var result=message.data.blankCopy;
	    homeworkSanatize(result);
	    resultsParse(result);
           $scope.hw=result; 
	   $scope.hw.questions.sort(function(a,b){
	       return a.index-b.index;
	   })
	...
	*/
	   ...
	   var validate = function (){
        $scope.error=false;
           for(var i =0; i<$scope.hw.questions.length;i++){
               if($scope.hw.questions[i].type=="ma"){
                   if($scope.hw.questions[i].response == "" ||  $scope.hw.questions[i].response.length ==0){
                        $scope.error=true;

                   }
               } else {
                   if($scope.hw.questions[i].response == ""){
                       $scope.error=true;
                   }
        ...
        $scope.flagQuestion = function(index){
            ...
            if($scope.hw.questions[index].flag == false){
            $scope.hw.questions[index].flag=true;
               } else {
                $scope.hw.questions[index].flag=false;
    ...
    */
    var homeworkResultsController = function($scope, $location, latinQuizModel,$routeParams){
	   ...
	   var result=message.data.results;
	   homeworkSanatize(result);
	   resultsParse(result);
	   console.log("rlt");
	   console.log(result);
	   ...
	   $scope.results.questions.sort(function(a,b){
	       return a.index-b.index;
	   })
       });
	//go back to the student login
	...
	$scope.goToBlankCopy=function(){
	    $location.path("/blankCopy/"+$routeParams.homeworkid);
	}
   ...
   var teacherClassController = function($scope, $location, latinQuizModel,$routeParams){
	   ...
	   angular.element(window).bind('resize',function(){
	       var height1=angular.element(".addQuestionDiv")[0].offsetHeight;
	       var height2=angular.element(".questionDiv")[0].offsetHeight;
	       if(height1>height2){
		   var parseHeight=height1+"px";
		   $scope.sizeCss={'height':parseHeight};
	       }
	       else{
		   var parseHeight=height2+"px";
		   $scope.sizeCss={'height':parseHeight};
	       }
	       console.log($scope.sizeCss);
	       console.log($scope);
	       $scope.$digest();
	   })
	   */
	   ...
	   $location.path("/teacherStudentOverview/"+$routeParams.classId+"/"+studentId);
       }
       
       $scope.goToQuestionOverview=function(questionIndex){
	   $location.path("/questionOverview/"+$routeParams.classId+"/"+$scope.displayedHw._id+"/"+questionIndex);
       }
   ...
   var teacherClassEditController = function($scope, $location, latinQuizModel,$routeParams){
	...
	classInfo:{
	   ...
	   $scope.class.chapters.forEach(function(chapter){
	       chapter.homework.forEach(function(hw){
		   //console.log(hw);
               var testDate =latinQuizModel.dateToString(new Date(hw.dueDate));
		   if(testDate!=hw.editableDueDate){
		       console.log(latinQuizModel.dateToString(new Date(hw.dueDate)));
		       console.log(hw.editableDueDate);
		       console.log("if true");
	   ...
	   $scope.dueDateToChange.forEach(function(hw){
           ...
           console.log("HELLO WOrld",hw.editableDueDate, typeof hw.editableDueDate);
	   ...
	   latinQuizModel.setDateOfHw($routeParams.classId,arrayOfHws).then(function(message){
	       console.log(message.data);
	       $scope.class=message.data.classInfo;	       
	       $scope.class.chapters.forEach(function(chapter){
		   $scope.showStuff(chapter);
	   ...
	   chapter.homework.forEach(function(hw){
	       hw.hide=chapter.hide;
	      hw.editableDueDate=latinQuizModel.dateToString(new Date(hw.dueDate));
	       /*
	   chapter.homework.forEach(function(hw){
	       hw.hide=chapter.hide;
	       hw.editableDueDate=hw.editableDueDate=latinQuizModel.dateToString(hw.date);
	       */
           })
	   $scope.adjustPic(chapter);
       }
       ...
       //function for setting what homework will be deleted
       //
       $scope.tryDeleteHomework=function(chapt,hw){
	   $scope.storeChapt=chapt;
	   $scope.deleteHw=hw;
	   $scope.dltHide2=false;
	   $scope.dltHide=true;
       }

       $scope.deleteHomework=function(){
	   latinQuizModel.deleteHomework($routeParams.classId,$scope.storeChapt._id,$scope.deleteHw._id).then(function(message){
	       $scope.class=message.data.classInfo;
	       $scope.dltHide2=true;
	       $scope.deleteHw="";
	       $scope.storeChapt="";
	   ...
	   if($scope.newChapterName=='' || $scope.newChapterName==undefined){
	       $scope.errorMessage="Every chapter must have a name";
	...
	$scope.changePassword=function(){
		...
		$scope.displayMessage="New and reentered passwords did not match";
		$scope.messageClass="warningMessage";
		$scope.hideMessage=false;
	    }
	}
	//go back to the student dashboard
...
var blankTrueFalse = function(){
	...
	//page for teacher to view all of the questions of a homework and see how the students did
	    .when("/questionOverview/:classId/:hwId/:questionId", {
